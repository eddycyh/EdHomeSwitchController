    ...
    if (r < 1 || t[0].type != JSMN_OBJECT) {
        ...
        for (int i = 1; i < r; i++) {
            if (jsoneq(body, &t[i], "humidity") == 0) {
                ...
                i++;
            } else if ((jsoneq(body, &t[i], "id") == 0) && (firstEncounter == false)) {
		str_length = t[i+1].end - t[i+1].start;
                memcpy(subbuff, body + t[i+1].start, str_length);
                subbuff[str_length] = '\0';
                weatherID = atoi(subbuff);
		firstEncounter = true;
	    }
        }
	ESP_LOGI(TAG, "Temperature %f", weather.temperature - 273);
	ESP_LOGI(TAG, "Humidity %d", weather.humidity);
	ESP_LOGI(TAG, "Pressure %f", weather.pressure);
	ESP_LOGI(TAG, "Weather ID %d", weatherID);
        return true;
    }
}
